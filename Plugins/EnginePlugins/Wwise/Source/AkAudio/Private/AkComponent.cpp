...
UAkComponent::UAkComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
    ...
    this->RoomComponentRefreshInterval = 0.33f;
...
bool UAkComponent::Seek(float Percent, UAkAudioEvent* AkEvent, const FString& in_EventName) {
    return false;
}

bool UAkComponent::Resume(UAkAudioEvent* AkEvent, int32 FadeTimeMs) {
    return false;
}
...
int32 UAkComponent::PostAssociatedAkEventCullDistance(int32 CallbackMask, const FOnAkPostEventCallback& PostEventCallback) {
    return 0;
}
...
int32 UAkComponent::PostAkEventWithMusicCallback(UAkAudioEvent* AkEvent) {
    return 0;
}

int32 UAkComponent::PostAkEventCullDistance(UAkAudioEvent* AkEvent) {
    return 0;
}
...
bool UAkComponent::Pause(UAkAudioEvent* AkEvent, int32 FadeTimeMs) {
    return false;
}
